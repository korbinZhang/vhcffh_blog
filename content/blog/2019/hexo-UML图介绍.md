...
description: 本文全面介绍了统一建模语言（UML）中的各种图及其在软件开发过程中的应用。内容涵盖了用例视图（用例图）、设计视图（类图、对象图）、进程视图（序列图、协作图、状态图、活动图）、实现视图（构件图）和拓扑视图（部署图）。文章还总结了这些UML图在需求、分析、设计、实现、测试等不同软件开发阶段所扮演的角色。
---
