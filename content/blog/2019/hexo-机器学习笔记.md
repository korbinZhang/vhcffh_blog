...
description: "本文是一篇关于机器学习评估指标的学习笔记。内容分为两部分：第一部分详细介绍了分类任务中常用的评估指标，包括准确率、精确率、召回率以及它们的调和均值F1分数。第二部分则聚焦于自然语言处理领域的序列评估指标，讲解了用于机器翻译的BLEU和用于自动摘要的ROUGE（特别是ROUGE-N和ROUGE-L）的计算方法和核心思想。"
---
